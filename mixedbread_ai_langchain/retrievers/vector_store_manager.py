...
# Using unified client approach through MixedbreadClient
...
class MixedbreadVectorStoreManager(SerializationMixin, AsyncMixin, ErrorHandlingMixin):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Use unified client approach - expose both sync and async clients
        self.client = self._client.client
        self.aclient = self._client.async_client
    ...
    def get_vector_store_file_info(
        ...
        try:
            logger.info(
                f"Getting file info {file_id} from vector store {vector_store_id}"
            )
