...
class SerializationMixin:
        ...
        for attr in dir(self):
            if not attr.startswith('_') and hasattr(self, attr):
                ...
                if (not callable(value) and 
                    attr not in ['client', 'async_client', 'api_key', 'base_url', 'timeout', 'max_retries']):
    ...
    def from_dict(cls: Type, data: Dict[str, Any]):
        ...
        if isinstance(api_key, dict):
            init_params["api_key"] = Secret.from_dict(api_key)
...
class AsyncMixin:
    ...
    def _ensure_async_client(self):
        ...
        if not hasattr(self, '_client') or not hasattr(self._client, 'async_client'):
            raise RuntimeError("Async client not available. Component must inherit from MixedbreadClient.")
...
class ErrorHandlingMixin:
    ...
    def _handle_api_error(self, error: Exception, operation: str, fallback_result: Any = None):
        """
