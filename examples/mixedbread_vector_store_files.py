"""
Vector Store File Retrieval Example

This example demonstrates how to search for entire files (with optional chunk content)
across vector stores using the enhanced MixedbreadVectorStoreFileRetriever.
"""
import asyncio
    ...
    try:
        ...
        for i, doc in enumerate(documents, 1):
            ...
            if chunk_details:
                ...
                for j, chunk_info in enumerate(chunk_details[:2], 1):  # Show first 2 chunks
    ...
    for query in queries:
        ...
        try:
            ...
            for i, doc in enumerate(file_results, 1):
                ...
                print(f"    {i}. {filename}")
                print(f"       Score: {score:.3f} | ID: {file_id} | Created: {created_at}")
...
async def async_file_search():
    ...
    async def search_files_with_convenience(query):
        try:
            ...
            filenames = [file.filename for file in response.data if hasattr(file, 'filename')]
...
def comparison_example():
    ...
    try:
        ...
        print(f"  Chunk search: {len(chunk_results)} chunks from {len(chunk_files)} files")
