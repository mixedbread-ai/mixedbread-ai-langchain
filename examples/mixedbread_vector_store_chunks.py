...
def multi_store_search():
    ...
    for query in queries:
        ...
        try:
            ...
            print(
                f"  Found {len(results)} chunks across {len(multi_retriever.vector_store_ids)} stores"
            )
...
def advanced_search_options():
    ...
    advanced_retriever.update_search_options(score_threshold=0.8, return_metadata=True)
...
def error_handling_example():
    ...
    retriever = MixedbreadVectorStoreRetriever(
        vector_store_ids=["invalid_store_id"], top_k=5
    )
    ...
    valid_retriever = MixedbreadVectorStoreRetriever(
        vector_store_ids=["valid_store_id"], top_k=5  # Replace with valid ID
    )
